...
class GetData(object):
    ...
    def load_and_save_users(cls,vk_connector, age_from,age_to,sex,fields=None):
        ...
        try:
            if answer['count']:
                ...
                ObjectStorage().save_obj(answer['items'], './data/{0}_user_from_{1}_to_{2}_sex_{3}'.format(user_len, age_from, age_to,sex))
    ...
    def load_and_save_walls(cls,vk_connector,uids,uids_from_range_cnt=150,post_cnt_to_extract=100):
        ...
        for uid in uids_lst:
            ...
            if  counter % 200 == 0:
                time.sleep(5)
        ObjectStorage().save_obj(users_wall_dict, './data/users_wall_posts')
        #чтобы недопустить дубликатов по id
        uids_lst = list(set(uids_lst))
        ObjectStorage().save_obj(uids_lst, './data/itog_uids_for_analysis')
